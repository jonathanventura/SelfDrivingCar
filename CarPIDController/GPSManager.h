...
{
public:
  GPSManager( int rx_pin, int tx_pin )
  : rx_pin_(rx_pin),
  tx_pin_(tx_pin),
  swserial_(rx_pin,tx_pin),
  time_(0),
  lat_(0),lon_(0),
  speed_(0),
  heading_(0)
  ...
  void setup()
  {
    ...
    if ( tx_pin_ != 255 ) pinMode( tx_pin_, OUTPUT );
    swserial_.begin(9600);

    if ( tx_pin_ != 255 ) configureGPS();
  }
  ...
  bool loop()
  {
    ...
    while ( swserial_.available() )
    {
      ...
      {
        if ( gps_.location.isValid() && time_ != gps_.time.value() ) {
          time_ = gps_.time.value();
          lat_ = gps_.location.lat();
          lon_ = gps_.location.lng();
          speed_ = gps_.speed.mps();
  ...
  {
    return gps_.distanceBetween( lat_, lon_, lat, lon );
  }

  double courseTo( double lat, double lon )
  {
    return gps_.courseTo( lat_, lon_, lat, lon );
  }
private:
  void configureGPS()
  {
    ...
    //ON  = 0x01
    //
    byte settingsArray[] = {0x04, 0xC8, 0x00, 0x00, 0x25, 0x00, 0x10, 0x00, 0x00, 0x10, 0x10}; //
  ...
  void configureUblox(byte *settingsArrayPointer) {
    ...
    byte setSave[] = {0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x1D, 0xAB};
  ...
  //  gpsSetSuccess = 0;
  //  while(gpsSetSuccess < 3 ) {
  //    Serial.print("Saving all settings ");
  //    sendUBX(setSave, sizeof(setSave));
  //    gpsSetSuccess += getUBX_ACK(&setSave[2]);
  //    if (gpsSetSuccess == 10) gpsStatus[7] = true;
  //    if (gpsSetSuccess == 5 | gpsSetSuccess == 6) gpsSetSuccess -= 4;
  //  }
  //  if (gpsSetSuccess == 3) Serial.println("Save all settings Failed!");
  }
  ...
  void setBaud(byte baudSetting) {
    if (baudSetting == 0x12) swserial_.begin(4800);
    if (baudSetting == 0x4B) swserial_.begin(19200);
    if (baudSetting == 0x96) swserial_.begin(38400);
    if (baudSetting == 0xE1) swserial_.begin(57600);
    if (baudSetting == 0xC2) swserial_.begin(115200);
    if (baudSetting == 0x84) swserial_.begin(230400);
  }
