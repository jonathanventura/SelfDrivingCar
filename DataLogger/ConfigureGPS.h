...
boolean gpsStatus[] = {false, false, false, false, false, false, false, false};
unsigned long start;
...
void configureGPS()
{
  ...
  //ON  = 0x01
  //
  byte settingsArray[] = {0x04, 0xC8, 0x00, 0x00, 0x25, 0x00, 0x01, 0x00, 0x00, 0x01, 0x01}; //
...
byte getUBX_ACK(byte *msgID) {
  ...
  int i = 0;
  while (1) {
    if (gGPSSerial.available()) {
      incoming_char = gGPSSerial.read();
...
void setBaud(byte baudSetting) {
  if (baudSetting == 0x12) gGPSSerial.begin(4800);
  if (baudSetting == 0x4B) gGPSSerial.begin(19200);
  if (baudSetting == 0x96) gGPSSerial.begin(38400);
  if (baudSetting == 0xE1) gGPSSerial.begin(57600);
  if (baudSetting == 0xC2) gGPSSerial.begin(115200);
  if (baudSetting == 0x84) gGPSSerial.begin(230400);
}
